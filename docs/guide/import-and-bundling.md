...
```js
const { Textbox, Button } = require('egenerui')
require('egenerui/style.css')
```

## 按需导入与 Tree-shaking
- 建议使用 `import { ... } from 'egenerui'` 命名导入，未使用的导出在生产构建中会被移除。
- 生产模式下构建更容易触发 Tree-shaking（Vite/webpack 默认启用）。
- 避免使用子路径导入，如 `egenerui/components/...`，请从包入口导入。

## 样式引入策略
- 模块方式：`import 'egenerui/style.css'`，适合 ESM/CJS 工程。
- CDN 方式：
...
## 常见问题排查
- 子路径导入报错：改为从包入口导入 `import { Button } from 'egenerui'`。
- Tree-shaking 无效：开启生产构建并使用命名导入；避免在运行时动态引用整个包。
- 样式未生效：确认已引入 `egenerui/style.css` 或正确加载 CDN 样式。
